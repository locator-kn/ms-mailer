...
function sendGenericMail(message, next) {
    ...
    Joi.validate(message.data, validation.mailAdress, (err, user) => {

        if (err) {
            return util.validationError(err, 'send easter mail service', next);
        }

        renderMail(EASTER_ACTION_TEMPLATE, {name: user.name, mail: user.mail})
            .then(mail => mailgun.sendMailToMailgun(user, user.mail, 'Ahoi ' + user.name + '!'))
            .then(() => next(null, {ok: true}))
            .catch(err => {
